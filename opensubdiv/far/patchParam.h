...
struct PatchParam {
    ...
    ///
    constexpr void Set(Index faceid, short u, short v,
                       unsigned short depth, bool nonquad,
                       unsigned short boundary, unsigned short transition,
                       bool regular = false);

    /// \brief Resets everything to 0
    constexpr void Clear() { field0 = field1 = 0; }

    /// \brief Returns the faceid
    constexpr Index GetFaceId() const { return Index(unpack(field0,28,0)); }
    ...
    /// the first corner of the patch
    constexpr unsigned short GetV() const { return (unsigned short)unpack(field1,10,12); }

    /// \brief Returns the transition edge encoding for the patch.
    constexpr unsigned short GetTransition() const { return (unsigned short)unpack(field0,4,28); }

    /// \brief Returns the boundary edge encoding for the patch.
    constexpr unsigned short GetBoundary() const { return (unsigned short)unpack(field1,5,7); }

    /// \brief True if the parent base face is a non-quad
    constexpr bool NonQuadRoot() const { return (unpack(field1,1,4) != 0); }
    ...
    constexpr float GetParamFraction() const;
    ...
    constexpr void Normalize( REAL & u, REAL & v ) const;
    template <typename REAL>
    constexpr void NormalizeTriangle( REAL & u, REAL & v ) const;
    ...
    ///
    template <typename REAL>
    constexpr void Unnormalize( REAL & u, REAL & v ) const;
    template <typename REAL>
    constexpr void UnnormalizeTriangle( REAL & u, REAL & v ) const;

    /// \brief Returns if a triangular patch is parametrically rotated 180 degrees
    constexpr bool IsTriangleRotated() const;

    /// \brief Returns whether the patch is regular
    constexpr bool IsRegular() const { return (unpack(field1,1,5) != 0); }

    unsigned int field0:32;
    unsigned int field1:32;

private:
    constexpr unsigned int pack(unsigned int value, int width, int offset) const {
        return (unsigned int)((value & ((1<<width)-1)) << offset);
    }

    constexpr unsigned int unpack(unsigned int value, int width, int offset) const {
